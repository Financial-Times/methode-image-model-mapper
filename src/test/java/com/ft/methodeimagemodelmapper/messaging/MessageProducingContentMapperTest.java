...
public class MessageProducingContentMapperTest {
    ...
    public void thatMessageIsProducedAndSent() throws Exception {
        ...
        when(delegate.mapImageModel(any(EomFile.class), eq(PUBLISH_REF), eq(lastModified))).thenReturn(content);

        Content actual = mapper.mapImageModel(incoming, PUBLISH_REF, lastModified);
    ...
    public void thatNoMessageIsSentWhenObjectMapperFails()
        ...
        try {
            mapper.mapImageModel(incoming, PUBLISH_REF, lastModified);
        } finally {
